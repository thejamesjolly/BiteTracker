...
/*#define WINDOW_SIZE 150 /* 30 sec @ 15 Hz // NOW PASSED IN AS PARAMETER*/
...

{
  ...
  if (argc > 2)
  {
    ...
    while (i < argc)
    {
      ...
      /*  Commented out of this version to allow for variable model names
      else if (strcmp(argv[i], "-fold") == 0)
        fold = atoi(argv[i + 1]);
      */
      else if (strcmp(argv[i], "-m_name") == 0)
        strcpy(model_name, argv[i + 1]);
      else if (strcmp(argv[i], "-start") == 0)
        start = atoi(argv[i + 1]);
      else if (strcmp(argv[i], "-end") == 0)
        end = atoi(argv[i + 1]);
      else if (strcmp(argv[i], "-ws") == 0) /*Window Size */
  ...
  { /* neural network classifier */
    ...
    TensorFlowInit(model_name,WINDOW_SIZE);
    //printf("Initializing Model Success\n");
	...
	** t0[x y z Y P R]  t1[x y z Y P R]  t2[x y z Y P R] ... */
      ...
      bites_in_window = TensorFlowClassify(windowdata,WINDOW_SIZE);
      if (0)
        printf("%f\n", bites_in_window);
      if (1)
...
/*#define INPUT_LENGTH 150 Now passed as input/* amount of data per input pattern */
                        /* 15 Hz x 30 sec = 450 */
...
void TensorFlowInit(char* model_name, int INPUT_LENGTH)

{
  ...
  /*if (fold == 0)
    ...
    sprintf(model_filename, "model-5b-fold%d.pb", fold);
  */
  //printf("\tOpening %s model file...\n",model_name);
  strcpy(model_filename, model_name);
...
float TensorFlowClassify(float *sampledata, int INPUT_LENGTH)
